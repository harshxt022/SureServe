...
const RegisterProviderPage = () => {
  ...
  const handleRegister = async (e) => {
    ...
    if (!formData.name || !formData.email || !formData.password || !formData.main_profession || !formData.phone) {
      setIsError(true);
          ...
          <div className="bg-white rounded-3xl p-8 border border-[#E8E8E4] shadow-[0_8px_32px_rgba(0,0,0,0.04)]">
            <form id="registerForm" className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={handleRegister}>
              ...
              <div className="space-y-5">
                ...
                <div>
                  <label className="block text-sm font-medium text-[#1A1A1A] mb-2">Main Profession / Service</label>
                  <select id="main_profession" required
                    value={formData.main_profession} onChange={handleChange}
                    className="w-full px-4 py-3 rounded-xl bg-[#FAFAF8] border border-[#E8E8E4] focus:ring-2 focus:ring-[#4F46E5] focus:border-transparent outline-none transition-all text-[#1A1A1A] text-sm appearance-none">
